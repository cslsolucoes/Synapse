...
**Localizacao:** `.cursor/Templates/form-units/`

Templates para gerar pares de arquivos de formulario Delphi e FPC/Lazarus.
O nome dos arquivos gerados e igual ao nome da `unit` declarada no `.pas`.
...
> O `.pas` e nomeado pelo `{UNIT_NAME}` — o sufixo `.vcl.`, `.fmx.` ou `.lcl.` faz parte
> apenas do nome do template para distingui-los, NAO aparece no arquivo gerado.

---

## Diferencas entre os frameworks
...
- 1.0.0 (31/03/2026): Criacao — templates VCL (.dfm+.pas), FMX (.fmx+.pas) e LCL (.lfm+.pas).
