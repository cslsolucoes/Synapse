...
2. Qualificar completude (skill `delphi-fpc-docs-to-structured-code` quando existir no kit Developer).
...
| Skill | Quando invoca |
|-------|---------------|
| `delphi-fpc-orchestrator` | Orquestração de tarefas multi-módulo Delphi/FPC; qualificação de completude de escopo |
...
| `delphi-fpc-docs-to-structured-code` | Ao receber documentação canónica e converter em mapa de implementação para experts |
| `project-refactoring-compatibility-policy` | Antes de qualquer tarefa que renomeie classes, units ou altere contratos públicos |
| `delphi-fpc-architecture-and-design` | Ao avaliar impacto arquitetural de tarefas multi-módulo antes de delegar |
...
- Não executa renomeações de classes ou units sem antes invocar `project-refactoring-compatibility-policy`.
...
- **`delphi-fpc-orchestrator`** — `.cursor/skills/delphi-fpc-orchestrator_V1.0.1/SKILL.md` (quando disponível); caso contrário **documentation-project-expert**.
