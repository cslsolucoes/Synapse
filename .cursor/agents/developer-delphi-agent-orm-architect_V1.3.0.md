...
| `project-refactoring-compatibility-policy` | Antes de renomear classes, métodos ou units ORM — obrigatório mesmo quando pedido direto |
...
- Não toma decisões de refactoring de nomes de classes ou métodos sem executar `project-refactoring-compatibility-policy` primeiro.
...
| Humano | Renomeação de classes/métodos cross-module, remoção de modo de conexão, quebra de compatibilidade intencional | Executar `project-refactoring-compatibility-policy` e escalar decisão ao utilizador |
...
| Renomear classes ORM sem executar a política de refactoring | Quebra compatibilidade silenciosamente; consumidores externos falham sem aviso | Executar `project-refactoring-compatibility-policy` antes de qualquer renomeação |
...
- Refactorings de nomes são sempre precedidos da execução de `project-refactoring-compatibility-policy`, com decisão explícita registrada.
