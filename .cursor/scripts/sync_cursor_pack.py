...
Usa pack-inventory.json para sync seletivo por versão semântica.
...
PACK_FILES = ['README.md', 'VERSION.md', 'config.json', 'pack-inventory.json']
...
# ---------------------------------------------------------------------------
# Inventário — helpers para sync seletivo por versão
# ---------------------------------------------------------------------------

def load_inventory(cursor_dir: Path) -> dict:
    """Carrega pack-inventory.json ou retorna {}."""
    inv_path = cursor_dir / 'pack-inventory.json'
    if inv_path.exists():
        try:
...
def build_version_index(inventory: dict) -> dict:
    """Retorna {rel_path: version_str} das entradas do inventário."""
    index: dict = {}
    for entry in inventory.get('files', []):
        ...
        if ver:
            ...
            name   = entry.get('name', '')
            key    = (folder + '/' + name).lstrip('/')
            index[key] = ver
    return index

def _version_gt(a: str, b: str) -> bool:
    """True se versão a > b (SemVer simples x.y.z)."""
    try:
...
def run_gen_inventory(source_cursor: Path) -> None:
    """Roda gen_pack_inventory.py --incremental para atualizar o inventário da fonte."""
    gen_script = source_cursor / 'scripts' / 'gen_pack_inventory.py'
    if not gen_script.exists():
        print('  [AVISO] gen_pack_inventory.py não encontrado — inventário não atualizado')
        return
    try:
        result = subprocess.run(
            [sys.executable, str(gen_script), '--incremental'],
        ...
        if result.returncode != 0:
            ...
            print(f'  [AVISO] gen_pack_inventory.py retornou {result.returncode}' +
                  (f': {err}' if err else ''))
    except Exception as e:
        print(f'  [AVISO] Falha ao executar gen_pack_inventory.py: {e}')
...
def main() -> int:
    ...
    # Carregar inventário da fonte para índice de versões
    src_inventory  = load_inventory(source_cursor)
    src_versions   = build_version_index(src_inventory)
    if src_versions:
        print(f'[OK] Inventário fonte carregado: {len(src_versions)} entradas com versão')
    print()
    ...
    for dest_path_str in args.dest:
        ...
        # Carregar inventário do destino para comparação de versões
        dst_inventory = load_inventory(dest_cursor)
        dst_versions  = build_version_index(dst_inventory)
    ...
    # Atualizar inventário da fonte após sync (se não houve erros graves)
    if not args.whatif and global_state.errors == 0:
        print()
        print('  --- Atualizando inventário da fonte ---')
        run_gen_inventory(source_cursor)
