...
Ponto de entrada para qualquer consulta sobre o módulo ActiveDirectoryORM — integração LDAP/Active Directory para GestorERP. Esta skill não executa diretamente — seleciona a skill especialista correta da família `developer-delphi-active-directory-*`.
...
- Documentação técnica ou geração de docs → `documentation-orchestrator`
- Convenções ProvidersORM (conexão SQL, ORM, pool) → `project-orchestrator`
- Delphi genérico (linguagem, RTL, patterns) → `developer-delphi-orchestrator`
