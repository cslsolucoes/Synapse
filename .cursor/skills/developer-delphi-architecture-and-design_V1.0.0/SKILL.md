...
## When NOT to use
- Não usar para tuning de build/compilação → use `developer-delphi-build-cross-compiler`.
- Não usar para diagnóstico de exceções ou rastreamento de erros → use `developer-delphi-error-handling-and-diagnostics`.
- Não usar para otimização de performance ou gestão de memória → use `developer-delphi-performance-and-memory`.
...
| `developer-delphi-language-core` | Antes de modelar generics, RTTI ou anonymous methods nos contratos |
...
- 1.0.0 (09/04/2026): Reorganização §17 — skill movida de `delphi-fpc-architecture-and-design`; novo prefixo canônico `developer-delphi`. Conteúdo idêntico ao V1.1.0 de origem; cross-references atualizadas para prefixo `developer-delphi-*`.
