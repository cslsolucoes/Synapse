...
|-----------|-------------|---------|
| **-h** | Host | `-h 127.0.0.1` ou `-h 10.100.2.3` |
...
```bat
mysql -h 10.100.2.3 -P 3306 -u root -p nome_do_banco
```

Executar script SQL:

```bat
mysql -h 10.100.2.3 -P 3306 -u root -p nome_do_banco < Data\script.sql
```
...
```bat
psql -h 10.100.2.3 -p 5432 -U postgres -d Habil -W
```
...
| Parâmetro | Significado | Exemplo |
|-----------|-------------|---------|
| **-S** | Servidor (host ou host,porta) | `-S 10.100.2.3` ou `-S 10.100.2.3,1433` |
...
```bat
sqlcmd -S 10.100.2.3,1433 -U sa -P 1qazxsw2@! -d Habil
```

Executar script:

```bat
sqlcmd -S 10.100.2.3,1433 -U sa -P 1qazxsw2@! -d Habil -i Data\script.sql
```

Com path completo (ajustar versão/caminho):

```bat
sqlcmd -S 10.100.2.3 -U sa -P 1qazxsw2@! -d Habil
```
...
| **Comando exemplo** | `mysql -h 10.100.2.3 -P 3306 -u root -p nome_do_banco` |
...
| **Comando exemplo** | `sqlcmd -S 10.100.2.3,1433 -U sa -P 1qazxsw2@! -d Habil` |
