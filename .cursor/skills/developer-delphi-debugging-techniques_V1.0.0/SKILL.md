...
- Não usar para modelar hierarquia de exceções → use `developer-delphi-error-handling-and-diagnostics`.
- Não usar para otimização de performance ou profiling → use `developer-delphi-performance-and-memory`.
- Não usar para configurar build pipelines ou compilação → use `developer-delphi-build-cross-compiler`.
...
|-------|-----------------------|
| `developer-delphi-error-handling-and-diagnostics` | Antes de interpretar stack traces e exceções capturadas |
| `developer-delphi-performance-and-memory` | Quando o problema envolver consumo de memória além de leaks simples |
