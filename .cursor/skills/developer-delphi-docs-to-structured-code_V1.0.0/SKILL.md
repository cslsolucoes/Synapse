...
- Não usar para decisões de arquitetura macro → use `developer-delphi-architecture-and-design`.
- Não usar para diagnóstico de exceções → use `developer-delphi-error-handling-and-diagnostics`.
- Não usar para build/compilação isolada → use `developer-delphi-build-cross-compiler`.
...
|-------|-----------------------|
| `developer-delphi-architecture-and-design` | Antes de mapear módulos/interfaces quando o escopo envolver design de camadas |
...
- 1.0.0 (09/04/2026): Reorganização §17 — skill movida de `delphi-fpc-docs-to-structured-code`; novo prefixo canônico `developer-delphi`. Conteúdo idêntico ao V1.1.0 de origem; cross-references atualizadas para prefixo `developer-delphi-*`.
