...
- Não usar para desenho de arquitetura completa → use `developer-delphi-architecture-and-design`.
- Não usar para definir estratégia de testes e quality gates → use `developer-delphi-testing-and-quality`.
- Não usar para otimização de memória ou detecção de leaks → use `developer-delphi-performance-and-memory`.
...
|-------|-----------------------|
| `developer-delphi-architecture-and-design` | Antes de definir hierarquia de exceções que abrange múltiplos módulos |
| `developer-delphi-language-core` | Antes de usar generics ou RTTI em exceções customizadas |
...
begin
  try
    raise Exception.Create('OK -- developer-delphi-error-handling-and-diagnostics');
  except
    on E: Exception do
      WriteLn(E.Message);
  end;
...
- 1.0.0 (09/04/2026): Reorganização §17 — skill movida de `delphi-fpc-error-handling-and-diagnostics`; novo prefixo canônico `developer-delphi`. Conteúdo idêntico ao V1.1.0 de origem; cross-references atualizadas para prefixo `developer-delphi-*`.
