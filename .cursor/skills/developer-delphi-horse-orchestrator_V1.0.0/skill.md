...
| --- | --- |
| Configurar servidor Horse, rotas GET/POST/PUT/DELETE, request/response, grupos | `developer-delphi-horse-core_V1.0.0` |
| Tratar exceções HTTP, `EHorseException`, callback de intercepção | `developer-delphi-horse-handle-exception_V1.0.0` |
| Autenticação HTTP Basic (`Authorization: Basic …`) | `developer-delphi-horse-basic-auth_V1.0.0` |
| Compressão GZIP/DEFLATE de respostas (`Accept-Encoding`) | `developer-delphi-horse-compression_V1.0.0` |
| CORS, preflight OPTIONS, origens permitidas, credenciais | `developer-delphi-horse-cors_V1.0.0` |
| Cache HTTP, ETag, `If-None-Match`, 304 Not Modified | `developer-delphi-horse-etag_V1.0.0` |
| Log de excepções em disco (ficheiro), auditoria de erros | `developer-delphi-horse-exception-logger_V1.0.0` |
| Autenticação JWT Bearer em rotas Horse, validar token, claims no request | `developer-delphi-horse-jwt_V1.0.0` |
| Infraestrutura de log — `THorseLoggerManager`, registar providers, formato | `developer-delphi-horse-logger_V1.0.0` |
| Provider de log para console/stdout (desenvolvimento) | `developer-delphi-horse-logger-console_V1.0.0` |
| Provider de log para ficheiro com rotação diária (produção) | `developer-delphi-horse-logger-logfile_V1.0.0` |
| Download/upload de ficheiros binários, `TStream`, `TFileReturn` | `developer-delphi-horse-octet-stream_V1.0.0` |
| Paginar listas JSON, `X-Paginate`, `limit`/`page`, summary wrapper | `developer-delphi-horse-paginate_V1.0.0` |
| Extrair IP real do cliente (proxy, Cloudflare, Nginx) | `developer-delphi-horse-clientip_V1.0.0` |
| JWT/JOSE base — gerar token, validar, `TJWT`, `TJOSE`, `TJWTClaims`; Swagger OpenAPI | `developer-delphi-horse-security_V1.0.0` |
| Consumir REST APIs externas, `TRequest`, HTTP client fluente, adapters CSV/DataSet | `developer-delphi-horse-client_V1.0.0` |
| Serializar DataSet↔JSON, `ToJSONArray`, `LoadFromJSON`, master-detail | `developer-delphi-horse-serialization_V1.0.0` |
