...
- Mudanças pequenas de um único domínio — usar a skill especializada diretamente (ex.: `developer-delphi-language-core` para ajuste de sintaxe).
- Consultas pontuais sobre uma API ou unit específica — usar `developer-delphi-rtl-and-units`.
- Análise isolada de performance sem impacto em arquitetura — usar `developer-delphi-performance-and-memory`.
...
| Skill | Versão | Responsabilidade |
| ----- | ------ | ---------------- |
| `developer-delphi-language-core` | V1.1.0 | **Orquestradora Linguagem** — sintaxe, estrutura de units, compatibilidade Delphi+FPC |
| `developer-delphi-language-types` | V1.1.0 | Tipos primitivos, records, enums, sets, type aliases, variant records |
| `developer-delphi-language-oop` | V1.1.0 | Classes, herança, interfaces, visibilidade, construtores, destrutores |
| `developer-delphi-language-generics` | V1.1.0 | TList\<T\>, TDictionary\<K,V\>, constraints, generic methods |
| `developer-delphi-language-rtti` | V1.1.0 | TRttiContext, atributos, inspeção de propriedades/métodos em runtime |
| `developer-delphi-language-advanced` | V1.1.0 | Closures, anonymous methods, operator overloading, helpers, ARC |
...
| `developer-delphi-patterns-creational` | V1.1.0 | Factory, Abstract Factory, Builder, Singleton, Prototype |
| `developer-delphi-patterns-structural` | V1.1.0 | Adapter, Bridge, Composite, Decorator, Facade, Proxy |
| `developer-delphi-patterns-behavioral` | V1.1.0 | Observer, Strategy, Command, Chain of Responsibility, Visitor, State |

### Família D — RTL (Runtime Library)

| Skill | Versão | Responsabilidade |
| ----- | ------ | ---------------- |
| `developer-delphi-rtl-and-units` | V1.1.0 | **Orquestradora RTL** — SysUtils, Math, DateUtils, IOUtils, System.* |
| `developer-delphi-rtl-collections` | V1.1.0 | TList, TObjectList, TDictionary, TQueue, TStack, THashSet |
| `developer-delphi-rtl-streams-io` | V1.1.0 | TStream, TFileStream, TMemoryStream, TReader, TWriter, JSON/XML I/O |
| `developer-delphi-rtl-strings` | V1.1.0 | TStringHelper, TStringBuilder, TRegEx, encoding, format, parse |

### Família E — Concorrência e Performance

| Skill | Versão | Responsabilidade |
| ----- | ------ | ---------------- |
| `developer-delphi-threading-basics` | V1.1.0 | TThread, Synchronize, Queue, seções críticas, mutexes básicos |
| `developer-delphi-threading-advanced` | V1.1.0 | TTask, PPL (Parallel Programming Library), TParallel.For, futures |
| `developer-delphi-performance-profiling` | V1.0.0 | AQTime, Sampling Profiler, instrumentação, gargalos de CPU/memória |
| `developer-delphi-performance-and-memory` | V1.0.0 | **Orquestradora Memória** — FastMM, leaks, alocações, gerenciamento ARC/manual |
| `developer-delphi-performance-and-architecture` | V1.0.0 | **Orquestradora Arquitetura de Performance** — cache, I/O assíncrono, pooling |
...
| Skill | Versão | Responsabilidade |
| ----- | ------ | ---------------- |
| `developer-delphi-build-cross-compiler` | V1.0.0 | dcc32, dcc64, fpc32, fpc64, cfg/opts, defines, flags CLI |
...
| Skill | Versão | Responsabilidade |
| ----- | ------ | ---------------- |
| `developer-delphi-error-handling-and-diagnostics` | V1.0.0 | Hierarquia de exceções, try/except/finally, logging, relatórios de erro |
...
| Skill | Versão | Responsabilidade |
| ----- | ------ | ---------------- |
| `developer-delphi-architecture-and-design` | V1.0.0 | DDD, Clean Architecture, SOLID, camadas, migração de monólito |
| `developer-delphi-architecture-modules` | V1.0.0 | Packages BPL, módulos runtime/designtime, dependências entre packages |
...
| `developer-delphi-assembly-x86-fundamentals` | V1.0.0 | Fundamentos x86: modos real/protegido, segmentos, endereçamento |
| `developer-delphi-assembly-registers` | V1.0.0 | Registradores gerais, de segmento, de controle (EAX..R15, flags) |
| `developer-delphi-assembly-instructions` | V1.0.0 | MOV, ADD, SUB, CMP, JMP, CALL, LEA e conjunto completo de instruções |
| `developer-delphi-assembly-stack-call` | V1.0.0 | Stack frame, PUSH/POP, ESP/EBP, prologue/epilogue de funções |
| `developer-delphi-assembly-calling-conventions` | V1.0.0 | register, pascal, cdecl, stdcall, fastcall — como Delphi/FPC escolhe |
| `developer-delphi-assembly-delphi-inline` | V1.0.0 | Bloco `asm...end` inline em Delphi, restrições e boas práticas |
| `developer-delphi-assembly-delphi-functions` | V1.0.0 | Funções puras em assembly, linkagem externa, exports |
...
| ------- | ---------------- |
| Linguagem/sintaxe | `developer-delphi-language-core` |
| RTL/units | `developer-delphi-rtl-and-units` |
| Build/CLI | `developer-delphi-build-cross-compiler` |
| Arquitetura/DI/migração | `developer-delphi-architecture-and-design` |
| Qualidade/testes | `developer-delphi-testing-and-quality` |
| Exceções/diagnóstico | `developer-delphi-error-handling-and-diagnostics` |
| Performance/memória | `developer-delphi-performance-and-memory` |
| Packaging/release | `developer-delphi-packaging-delivery` |
| UI FMX | `developer-delphi-fmx-layout` |
| Assembly inline | `developer-delphi-assembly-delphi-inline` |
...
| ----- | --------------------- |
| `developer-delphi-build-cross-compiler` | Antes de qualquer execução de build no fluxo orquestrado |
...
| Papel | Quem |
| ----- | ---- |
| Executor | `developer-delphi-orchestrator` (esta skill) |
