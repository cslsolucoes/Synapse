...
- Não usar para modelagem de linguagem → use `delphi-fpc-language-core`.
- Não usar para execução do build de compilação → use `developer-delphi-build-cross-compiler`.
- Não usar para definir estratégia de testes → use `delphi-fpc-testing-and-quality`.
...
|-------|-----------------------|
| `developer-delphi-build-cross-compiler` | Build Delphi e FPC deve estar limpo (zero erros/warnings) antes do empacotamento |
| `delphi-fpc-testing-and-quality` | Gates de testes devem estar aprovados antes de gerar o pacote de release |
...
- 1.0.0 (09/04/2026): Reorganização §17 — skill movida de `delphi-fpc-packaging-and-delivery`; novo prefixo canônico `developer-delphi`. Conteúdo V2 preservado (FileVersion 1.1.0 da origem). Referências internas atualizadas para nomes canônicos.
