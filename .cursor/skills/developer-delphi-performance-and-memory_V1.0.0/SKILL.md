---
name: developer-delphi-performance-and-memory
...
- Não usar para definir domínio funcional ou contratos de interface → use `developer-delphi-architecture-and-design`.
...
- Não usar para configurar build/compilação → use `developer-delphi-build-cross-compiler`.
...
|-------|-----------------------|
| `developer-delphi-build-cross-compiler` | Build limpo é pré-requisito antes de habilitar diagnósticos de memória |
...
begin
  ...
  try
    S.Add('OK -- developer-delphi-performance-and-memory');
    WriteLn(S[0]);
  finally
    S.Free;
  end;
...
- 1.0.0 (09/04/2026): Reorganização §17 — skill movida de `delphi-fpc-performance-and-memory`; novo prefixo canônico `developer-delphi`. Conteúdo idêntico ao V1.1.0 de origem; cross-references atualizadas para prefixo `developer-delphi-*`.
