---
name: developer-delphi-performance-profiling
...
- Não usar para decisões arquiteturais de pool de objetos ou threading → use `developer-delphi-performance-and-architecture`.
...
|-------|-----------------------|
| `developer-delphi-build-cross-compiler` | Build Release limpo e sem hints/warnings é pré-requisito |
| `developer-delphi-performance-and-memory` | Confirmar diagnóstico de leak antes de interpretar snapshots FastMM5 |
...
- Se a otimização alterar contrato público de interface, confirmar antes e executar `project-refactoring-compatibility-policy`.
