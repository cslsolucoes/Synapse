...
| Deploy de app Delphi em Linux | `developer-delphi-linux-servers_V1.0.0` | `SKILL.md` secção "Deploy Linux" |
| `dcclinux64`, compilar para Linux no Windows | `developer-delphi-linux-servers_V1.0.0` | `exemplos/dcclinux64_build_guide.md` |
| PAServer setup e configuração | `developer-delphi-linux-servers_V1.0.0` | `exemplos/paserver_linux_setup.md` |
| Daemon Linux (`fork`, `setsid`, `systemd`) | `developer-delphi-linux-servers_V1.0.0` | `templates/TEMPLATE_linux_console_daemon.pas` |
| Diferenças FPC vs Delphi no Linux (Posix vs BaseUnix) | `developer-delphi-linux-servers_V1.0.0` | `consultas_rapidas/posix_vs_fpc_linux.md` |
| Opções do compilador `dcclinux64` | `developer-delphi-linux-servers_V1.0.0` | `consultas_rapidas/dcclinux64_options.md` |
| Criar DLL / `.so` (projecto library) | `developer-delphi-shared-libraries_V1.0.0` | `exemplos/library_basic_exports.pas` |
| Problema de memory manager entre DLLs | `developer-delphi-shared-libraries_V1.0.0` | `exemplos/dll_memory_manager_guide.md` |
| Plugin system com interfaces (`IPlugin`) | `developer-delphi-shared-libraries_V1.0.0` | `exemplos/interface_plugin_host.pas` |
| `LoadLibrary` / `dlopen` (carregamento dinâmico) | `developer-delphi-shared-libraries_V1.0.0` | `exemplos/loadlibrary_consumer_win.pas` + `exemplos/dlopen_consumer_linux.pas` |
| Calling conventions (`stdcall` vs `cdecl`) | `developer-delphi-shared-libraries_V1.0.0` | `consultas_rapidas/calling_conventions_win_linux.md` |
| BPL vs DLL — quando usar cada um | `developer-delphi-shared-libraries_V1.0.0` | `consultas_rapidas/bpl_vs_dll_comparison.md` |
| `.dproj` multi-plataforma para library | `developer-delphi-shared-libraries_V1.0.0` | `consultas_rapidas/dproj_library_multiplatform.md` |
| Armadilhas de memory manager em DLLs | `developer-delphi-shared-libraries_V1.0.0` | `consultas_rapidas/memory_manager_pitfalls.md` |
| Empacotar e distribuir DLL (Inno Setup, MSIX) | `developer-delphi-packaging-delivery_V1.0.0` | `SKILL.md` secção "DLL deployment" |
| Build cross-platform (Win+Linux no mesmo pipeline) | `developer-delphi-build-cross-compiler_V1.0.0` | `SKILL.md` |
...
│   │   └─► developer-delphi-shared-libraries_V1.0.0                           │
...
└─► Linux? ─────────────────────────────────────────────────────────────────┐  │
    ...
    │   └─► developer-delphi-linux-servers_V1.0.0                          │  │
...
1. `developer-delphi-linux-servers_V1.0.0` — setup PAServer + compilação dcclinux64
   - Guia: `exemplos/paserver_linux_setup.md`
   - Guia de build: `exemplos/dcclinux64_build_guide.md`
2. `developer-delphi-linux-servers_V1.0.0` — estrutura daemon com daemonização POSIX
   - Template: `templates/TEMPLATE_linux_console_daemon.pas`
   - Signals: SIGTERM para shutdown graceful, SIGHUP para reload config
3. `developer-delphi-linux-servers_V1.0.0` → `templates/TEMPLATE_daemon.service`
   - Instalar como unidade systemd com `Restart=on-failure`
   - `EnvironmentFile` para configuração externa
4. `developer-delphi-linux-servers_V1.0.0` → `consultas_rapidas/linux_runtime_deps.md`
...
1. `developer-delphi-shared-libraries_V1.0.0` — definir interface `IPlugin` partilhada
...
2. `developer-delphi-shared-libraries_V1.0.0` — configurar projecto library multi-plataforma
...
3. `developer-delphi-shared-libraries_V1.0.0` — gerir memory manager
   - Guia: `exemplos/dll_memory_manager_guide.md`
   - Usar interfaces como boundary (sem ShareMem necessário)
4. `developer-delphi-linux-servers_V1.0.0` — compilar `.so` para Linux64
...
2. `developer-delphi-shared-libraries_V1.0.0` — plugin system via interfaces
...
| `developer-delphi-linux-servers_V1.0.0` | PAServer, dcclinux64, daemon POSIX, systemd, FPC Linux |
| `developer-delphi-shared-libraries_V1.0.0` | DLL/.so, exports, LoadLibrary/dlopen, plugin system, memory manager |

### Skills complementares (outras famílias)

| Skill | Âmbito | Quando usar |
|-------|--------|-------------|
| `developer-delphi-build-cross-compiler_V1.0.0` | Build Win+Linux no mesmo pipeline | CI/CD multi-plataforma |
...
| `developer-delphi-threading-basics_V1.1.0` | Threads, TThread, synchronização | Worker threads em serviços |
| `developer-delphi-threading-advanced_V1.1.0` | Thread pools, lock-free, ICS | Serviços de alta carga |
| `developer-delphi-architecture-modules_V1.0.0` | BPL, plugin architecture | Decisão BPL vs DLL |
