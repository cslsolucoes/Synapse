...
- Não usar para troubleshooting profundo de exceções em runtime → use `developer-delphi-error-handling-and-diagnostics`.
- Não usar para otimização de performance ou gestão de leaks → use `developer-delphi-performance-and-memory`.
- Não usar para execução de build isolada sem contexto de testes → use `developer-delphi-build-cross-compiler`.
...
|-------|-----------------------|
| `developer-delphi-build-cross-compiler` | Build limpo é pré-requisito antes de executar qualquer suite de testes |
| `developer-delphi-error-handling-and-diagnostics` | Exceções esperadas devem estar modeladas antes de escrever testes que as verificam |
...
- 1.0.0 (09/04/2026): Reorganização §17 — skill movida de `delphi-fpc-testing-and-quality`; novo prefixo canônico `developer-delphi`. Conteúdo idêntico ao V1.1.0 de origem; cross-references atualizadas para prefixo `developer-delphi-*`.
