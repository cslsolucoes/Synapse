...
- Para documentação técnica → `documentation-orchestrator`
- Para governança de processo → `governance-orchestrator`
- Para QA ou code review → `quality-orchestrator`
- Para implementação de features → `developer-delphi-orchestrator`
...
| `project-abrir-bancos-cli` | Acesso CLI a bancos (mysql, sqlite3, isql, psql) | Ao inspecionar ou manipular banco via terminal |
...
| Como abrir o banco SQLite pelo terminal? | `project-abrir-bancos-cli` |
