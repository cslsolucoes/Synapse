...
Esta skill aplica versionamento semântico (SemVer) ao **Providers.2.1.0 como produto público** — decide quando bumpar MAJOR, MINOR ou PATCH com base nas mudanças do release, atualiza `VERSION.md`, gera a tag de versão e registra a entrada no changelog. É a referência canónica para qualquer decisão de versão de produto no Providers.2.1.0. Não versiona artefatos internos do pack `.cursor/` (responsabilidade de `pack-versioning-policy`).
